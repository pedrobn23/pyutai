...
class DiscreteFactor(BaseFactor, StateNameMixin):
    ...
    def __init__(self, variables, cardinality, values, state_names={}):
        ...
        if isinstance(variables, str):
            raise TypeError(
                "Variables: Expected type list or array like, got string")
        ...
        # Set the state names
        super(DiscreteFactor, self).store_state_names(variables, cardinality,
                                                      state_names)
    ...
    def get_cardinality(self, variables):
        ...
        return {
            var: self.cardinality[self.variables.index(var)]
    ...
    def get_value(self, **kwargs):
        ...
        for variable in kwargs.keys():
            if variable not in self.variables:
                raise ValueError(
                    f"Factor doesn't have the variable: {variable}")
        ...
        for var in self.variables:
            if var not in kwargs.keys():
                ...
                try:
                    ...
                    logging.info(
                        f"Using {var} state as number instead of name.")
    ...
    def is_valid_cpd(self):
        ...
        """
        return np.allclose(
            self.to_factor().marginalize(self.scope()[:1],
                                         inplace=False).values.flatten("C"),
    ...
    def __eq__(self, other, atol=1e-08):
        ...
        """
        if not (isinstance(self, DiscreteFactor) and
                isinstance(other, DiscreteFactor)):
            return False
    ...
    def __hash__(self):
        ...
        for axis in range(phi.values.ndim):
            ...
            phi.values = phi.values.swapaxes(axis, exchange_index)
        return hash(
            str(sorted_var_hashes) + str(phi.values) + str(phi.cardinality) +
